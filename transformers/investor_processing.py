...
def transform_investors(data, *args, **kwargs):
    ...
    small_inv = data[0]
    big_inv = data[1]
    investors_existing = pd.DataFrame()
    
    if not args[0].empty: investors_existing = args[0]
    ...
    if not small_inv.empty and not big_inv.empty: 
        ...
        #Primary Organization URL_x, Primary Organization_x, Primary Job Title_x
        ...
        merged_investor = merged_investor.drop(['LinkedIn_y','Primary Job Title_y','Primary Organization_y','Primary Organization URL_y','Number of Lead Investments_y','Number of Investments_y','Number of Partner Investments_y','Location_y','Facebook_y','Founded Date','Founded Date Precision','Industry Groups','CB Rank (Person)_x','Full Name_y','CB Rank (Person)_y'], axis=1)
        
    merged_investor = merged_investor.reset_index(drop=True)
    
    if investors_existing.empty: unique_investors = merged_investor
    else:unique_investors = merged_investor[~merged_investor['Full Name URL'].isin(investors_existing['full_name_url'])]
